...
void Car::ResetStepContactState() {
	_internalState.wasOnGroundDuringStep = false;
}
...
void Car::_PreTickUpdate(GameMode gameMode, float tickTime, const MutatorConfig& mutatorConfig) {
	...
	{ // Update isOnGround
		const bool rawIsOnGround = numWheelsInContact >= 3;
		const bool useGroundContactGrace =
			!rawIsOnGround &&
			_internalState.isOnGround &&
			!_internalState.groundContactGraceUsed &&
			_internalState.hasJumped &&
			!_internalState.hasFlipped &&
			!_internalState.hasDoubleJumped;
		_internalState.isOnGround = rawIsOnGround || useGroundContactGrace;
		_internalState.groundContactGraceUsed = useGroundContactGrace;
		_internalState.wasOnGroundDuringStep |= _internalState.isOnGround;
	}

	float forwardSpeed_UU = _bulletVehicle.getForwardSpeed() * BT_TO_UU;
	const bool startsGroundJump =
		_internalState.isOnGround &&
		jumpPressed &&
		!_internalState.isJumping &&
		!_internalState.hasJumped;
	const bool startsWallJump = startsGroundJump && std::abs(GetUpDir().z) < 0.75f;
	_UpdateWheels(tickTime, mutatorConfig, numWheelsInContact, forwardSpeed_UU);

	_UpdateJump(tickTime, mutatorConfig, jumpPressed, _internalState.isOnGround);
	_UpdateAutoFlip(tickTime, mutatorConfig, jumpPressed);

	// Start flips before air torque so the first dodge tick can apply torque.
	_UpdateDoubleJumpOrFlip(
		tickTime,
		mutatorConfig,
		jumpPressed,
		forwardSpeed_UU,
	...
	if (_internalState.isJumping) {
		// Match RLBot's airborne state while a jump is active, even if wheel raycasts still touch.
		_internalState.isOnGround = false;
	}

	if (startsWallJump && _internalState.isJumping) {
		// RLBot does not appear to apply the wall suspension push on the same
		// tick as a wall jump. Keeping the cached raycast impulses here adds
		// extra outward speed and can turn the next wall-dash pulse into an air flip.
		for (int i = 0; i < _bulletVehicle.m_wheelInfo.size(); i++) {
			auto& wheel = _bulletVehicle.m_wheelInfo[i];
			wheel.m_raycastInfo.m_isInContact = false;
			wheel.m_raycastInfo.m_groundObject = nullptr;
			wheel.m_wheelsSuspensionForce = 0;
			wheel.m_impulse = btVector3(0, 0, 0);
		}
	}
	...
	_internalState.obsOnGround = _internalState.isOnGround && !_internalState.isJumping;
	...
	if (_internalState.jumpResetBlockTicks > 0)
		_internalState.jumpResetBlockTicks--;

	{ // Limit velocities before the physics step to match Rocket League timing more closely
		btVector3
...
void CarState::Deserialize(DataStreamIn& in) {
	...
	wasOnGroundDuringStep = isOnGround;
}
...
void Car::_UpdateBoost(float tickTime, const MutatorConfig& mutatorConfig, float forwardSpeed_UU) {
	...
	if (hasBoost) { // Update boosting timer
		if (_internalState.isBoosting) {
			if (controls.boost || _internalState.boostingTime < BOOST_MIN_TIME) {
				// Keep boosting
				_internalState.isBoosting = true;
			} else {
				_internalState.isBoosting = false;
			}
		} else {
			if (controls.boost) {
				// Start boosting
				_internalState.isBoosting = controls.boost;
			}
		}
	} else {
		_internalState.isBoosting = false;
	...
	if (_internalState.isBoosting) {
		_internalState.boostingTime += tickTime;
	} else {
		_internalState.boostingTime = 0;
	}
...
void Car::_UpdateJump(
	float tickTime,
	const MutatorConfig& mutatorConfig,
	bool jumpPressed,
	bool hasSurfaceJumpContact) {
	using namespace RLConst;
	const bool hadJumpedBeforeGroundReset = _internalState.hasJumped;
	const bool hadDodgeBeforeGroundReset = _internalState.hasFlipped || _internalState.hasDoubleJumped;
	if (_internalState.isOnGround && !_internalState.isJumping) {
		if (_internalState.hasJumped && _internalState.jumpTime < JUMP_MIN_TIME + JUMP_RESET_TIME_PAD) {
			...
			if (_internalState.hasJumped && !hadDodgeBeforeGroundReset)
				_internalState.jumpResetBlockTicks = 3;
	...
	if (_internalState.isJumping) {
		...
		hasSurfaceJumpContact &&
		jumpPressed &&
		!_internalState.hasJumped &&
		(!hadJumpedBeforeGroundReset || hadDodgeBeforeGroundReset) &&
		_internalState.jumpResetBlockTicks == 0) {
		// RLBot traces reject jump edges for the first two packets after has_jumped resets.
		// Start jumping
		_internalState.isJumping = true;
		_internalState.jumpTime = 0;
		_internalState.jumpResetBlockTicks = 0;
...
void Car::_UpdateAirTorque(float tickTime, const MutatorConfig& mutatorConfig, bool updateAirControl) {
	...
	const bool willApplyBoost =
		_internalState.boost > 0 &&
		(controls.boost || (_internalState.isBoosting && _internalState.boostingTime < BOOST_MIN_TIME));
	if (controls.throttle != 0 && !willApplyBoost)
		_rigidBody.applyCentralForce(GetForwardDir() * controls.throttle * THROTTLE_AIR_ACCEL * UU_TO_BT * CAR_MASS_BT);
}

void Car::_UpdateDoubleJumpOrFlip(
	float tickTime,
	const MutatorConfig& mutatorConfig,
	bool jumpPressed,
	float forwardSpeed_UU,
	bool startedGroundJump) {
	using namespace RLConst;

	float tickTimeScale = tickTime / (1 / 120.f);

	if (_internalState.isOnGround || startedGroundJump) {
		...
		const bool canUseJumpDodge =
			_internalState.jumpResetBlockTicks == 0 &&
			_internalState.hasJumped &&
			_internalState.airTimeSinceJump < DOUBLEJUMP_MAX_DELAY;
		const bool canUseResetDodge =
			_internalState.jumpResetBlockTicks == 0 &&
			!_internalState.hasJumped &&
			_internalState.airTime >= JUMP_MIN_TIME + JUMP_RESET_TIME_PAD &&
			_internalState.airTime < DOUBLEJUMP_MAX_DELAY;
		if (jumpPressed && (canUseJumpDodge || canUseResetDodge)) {
