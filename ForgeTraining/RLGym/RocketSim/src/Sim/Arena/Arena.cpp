...
namespace {
	constexpr float DEMO_COM_ANGLE_YAW = 45.572994f;
	constexpr float DEMO_COM_ANGLE_PITCH = 36.869896f;
	constexpr float BUMP_COM_ANGLE_YAW = 70.f;
	constexpr float BUMP_COM_ANGLE_PITCH = 36.869896f;
	constexpr float DEMO_FORWARD_SPEED_THRESHOLD = 1700.f;

	bool IsWithinForwardEllipticalCone(
		const Vec& carForward, const Vec& carRight, const Vec& carUp,
		const Vec& impactDir,
		float maxYawDeg, float maxPitchDeg,
		bool reverseForward) {
		constexpr float PROJECTION_FUDGE = 1.01f;
		constexpr float RAD_TO_DEG = 57.29578f;

		Vec forward = reverseForward ? -carForward : carForward;

		// Rocket League checks independent yaw and pitch slices of the COM impact direction.
		float dotRight = impactDir.Dot(carRight);
		float scaledRight = (dotRight >= 0.f) ? (dotRight / PROJECTION_FUDGE) : (dotRight * PROJECTION_FUDGE);
		Vec pitchPlaneDir = (impactDir - carRight * scaledRight).Normalized();
		float pitchDeg = acosf(RS_CLAMP(forward.Dot(pitchPlaneDir), -1.f, 1.f)) * RAD_TO_DEG;
		if (pitchDeg > maxPitchDeg)
			return false;

		float dotUp = impactDir.Dot(carUp);
		float scaledUp = (dotUp >= 0.f) ? (dotUp / PROJECTION_FUDGE) : (dotUp * PROJECTION_FUDGE);
		Vec yawPlaneDir = (impactDir - carUp * scaledUp).Normalized();
		float yawDeg = acosf(RS_CLAMP(forward.Dot(yawPlaneDir), -1.f, 1.f)) * RAD_TO_DEG;
...
void Arena::_BtCallback_OnCarCarCollision(Car* car1, Car* car2, btManifoldPoint& manifoldPoint) {
	...
	struct PendingAction {
		...
		Car* victim = NULL;
		bool isDemo = false;
		btVector3 bumpImpulseBT = {};
	};
	PendingAction pendingActions[2];
	int numPendingActions = 0;

	// Test collision both ways before applying actions so head-on mutual demos can be represented.
	for (int i = 0; i < 2; i++) {
		...
		if (state.vel.Dot(deltaPos) > 0) { // Going towards other car
			...
			if (speedTowardsOtherCar > otherCarAwaySpeed) { // Going towards other car faster than they are going away
				...
				bool isDemo;
				bool skipSpeedAndAngleChecks = false;
				switch (_mutatorConfig.demoMode) {
				case DemoMode::ON_CONTACT:
					isDemo = true; // BOOM
					skipSpeedAndAngleChecks = true;
					break;
				...
				float forwardSpeed = state.vel.Dot(carForward);
				bool reverseForward = forwardSpeed < 0.f;

				if (isDemo && !skipSpeedAndAngleChecks) {
					if (fabsf(forwardSpeed) < DEMO_FORWARD_SPEED_THRESHOLD)
						isDemo = false;
				}

				bool passed = skipSpeedAndAngleChecks && isDemo;
				if (!passed) {
					if (isDemo) {
						passed = IsWithinForwardEllipticalCone(
							carForward, carRight, carUp, dirToOtherCar,
							DEMO_COM_ANGLE_YAW, DEMO_COM_ANGLE_PITCH, reverseForward);

						if (!passed) {
							passed = IsWithinForwardEllipticalCone(
								carForward, carRight, carUp, dirToOtherCar,
								BUMP_COM_ANGLE_YAW, BUMP_COM_ANGLE_PITCH, reverseForward);
							if (passed)
								isDemo = false;
						}
					} else {
				...
				pendingActions[numPendingActions++] = action;
			}
		}
	}

	for (int i = 0; i < numPendingActions; i++) {
		...
		if (_carBumpCallback.func)
			_carBumpCallback.func(this, action.attacker, action.victim, true, _carBumpCallback.userInfo);
	}
	...
	for (int i = 0; i < numPendingActions; i++) {
		...
		if (action.attacker->GetState().isDemoed || action.victim->GetState().isDemoed)
			continue;
		...
		if (_carBumpCallback.func)
			_carBumpCallback.func(this, action.attacker, action.victim, false, _carBumpCallback.userInfo);
	}
}
...
Arena::Arena(GameMode gameMode, const ArenaConfig& config, float tickRate) : _mutatorConfig(gameMode), _config(config) {
	...
	{ // Initialize world
		...
		// RL tolerance does not need Bullet's default of 10 iterations.
...
BallState Arena::GetFutureBallState(const float time)
{
	if (time > 0.f)
	{
		// predTicks must be in PRED-arena ticks (the unit of numPredTicks and predData).
		// When ballPredTickRate differs from the main rate the two tick-times diverge,
		// so compute against the resolved pred tick-time rather than the outer tickTime.
		const float configRate = _config.ballPredTickRate;
		const float predRate = (configRate > 0.f) ? configRate : GetTickRate();
		const float predTickTime = 1.f / predRate;
		const int predTicks = static_cast<int>(time / predTickTime);

		if (!ballPredTracker)
			ballPredTracker = new BallPredTracker(this, predTicks);

		if (ballPredTracker)
		{
			// If ball prediction doesn't predict far enough into the future, update it with new maximum time
			if (predTicks > static_cast<int>(ballPredTracker->numPredTicks))
				ballPredTracker->SetNumPredTicks(predTicks);

			ballPredTracker->UpdatePredFromArena(this);

			return ballPredTracker->GetBallStateForTime(time);
		}
	}
	else
...
bool Arena::GetPredictedScore(float& outScoreTime, Vec& outScorePos) const
{
	if (ballPredTracker)
	{
		const int predScoreStepIndex = ballPredTracker->predScoreStepIndex;
		if (predScoreStepIndex > 0 && ballPredTracker->predData.size() > predScoreStepIndex)
		{
			outScoreTime = predScoreStepIndex * ballPredTracker->ballPredArena->tickTime;
			outScorePos = ballPredTracker->predData[predScoreStepIndex].pos;
			return true;
		}
	}
