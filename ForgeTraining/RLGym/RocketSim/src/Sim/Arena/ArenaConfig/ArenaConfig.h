...
struct ArenaConfig {
	...
	// Tick rate for the internal ball-prediction arena (BallPredTracker).
	// 0 (or any value <= 0) means "match the main arena tick rate" — backward-compatible default.
	// Lowering this (e.g. 60 when the main arena runs at 120) halves the number of
	// Bullet steps per repred at the cost of coarser prediction.
	// Runtime-only knob; deliberately excluded from Serialize/Deserialize so adding
	// it does not bump the wire format.
