...
public:
	...
	BallState GetFutureBallState(const float time);

	// Returns true if there is a predicted score, and outputs time to score in seconds and location where it scores
	bool GetPredictedScore(float& outScoreTime, Vec& outScorePos) const;
...
private:
	...
	// Shared ball prediction tracking
	RocketSim::BallPredTracker* ballPredTracker = nullptr;
};
