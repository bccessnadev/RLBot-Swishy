...
btVector3 Math::RandVec(const btVector3& min, const btVector3& max)
{
	if (max.isZero())
		return max;

	btVector3 randVec;
	randVec.setX(max.x() != 0.f ? Math::RandFloat(min.x(), max.x()) : 0.f);
	randVec.setY(max.y() != 0.f ? Math::RandFloat(min.y(), max.y()) : 0.f);
	randVec.setZ(max.z() != 0.f ? Math::RandFloat(min.z(), max.z()) : 0.f);
