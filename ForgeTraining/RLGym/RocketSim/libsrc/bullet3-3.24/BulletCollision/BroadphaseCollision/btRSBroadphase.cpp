...
namespace {
	bool rayAabbFilterEnabled = true;

	struct SegmentAabbFilter {
		btVector3 min;
		btVector3 max;

		SegmentAabbFilter(const btVector3& from, const btVector3& to) {
			min = from;
			max = from;
			min.setMin(to);
			max.setMax(to);

			const btVector3 epsilon(SIMD_EPSILON, SIMD_EPSILON, SIMD_EPSILON);
			min -= epsilon;
			max += epsilon;
		}

		bool Intersects(const btBroadphaseProxy* proxy) const {
			return TestAabbAgainstAabb2(min, max, proxy->m_aabbMin, proxy->m_aabbMax);
		}
	};
}

void btRSBroadphase::SetRayAabbFilterEnabled(bool enabled) {
	rayAabbFilterEnabled = enabled;
}

bool btRSBroadphase::GetRayAabbFilterEnabled() {
	return rayAabbFilterEnabled;
}
...
void btRSBroadphase::rayTest(const btVector3& rayFrom, const btVector3& rayTo, btBroadphaseRayCallback& rayCallback, const btVector3& aabbMin, const btVector3& aabbMax) {
	...
	SegmentAabbFilter rayAabbFilter(rayFrom, rayTo);

	if (rayLenSq < cellSizeSq) {

		Cell& cell = cells[GetCellIdx(rayFrom)];
		for (auto& otherProxy : cell.staticHandles)
			if (otherProxy->m_clientObject && (!rayAabbFilterEnabled || rayAabbFilter.Intersects(otherProxy)))
		...
		for (int i = 0; i <= m_LastHandleIndex; i++) {
			...
			if (!rayAabbFilterEnabled || rayAabbFilter.Intersects(proxy))
